...
class RecurrentPPO(RecurrentOnPolicyModel):
  ...
  def training_step(self, batch: RecurrentRolloutBatch, batch_idx: int) -> float:
    ...
    """
    batch_observations = batch.observations.squeeze(0)
    batch_actions = batch.actions.squeeze(0)
    batch_dones = batch.dones.squeeze(0)
    hidden_states = (batch.hidden_states[0].squeeze(0),
                     batch.hidden_states[1].squeeze(0))
    batch_advantages = batch.advantages.squeeze(0)
    batch_returns = batch.returns.squeeze(0)
    batch_values = batch.values.squeeze(0)
    batch_log_probs = batch.log_probs.squeeze(0)
    # Have to switch so the batch dimension is in the middle
    actions, log_probs, entropy, values, _ = self.act(
        batch_observations, hidden_states, batch_dones, batch_actions)
    ...
    if self.value_clip_range is None:
      ...
      values_pred = batch_values + torch.clamp(values - batch_values,
                                               -self.value_clip_range,
                                               self.value_clip_range)
    # Value loss using the TD(gae_lambda) target
    value_loss = F.mse_loss(batch_returns, values_pred)
    ...
    with torch.no_grad():
      ...
      approx_kl = torch.mean(batch_log_probs - log_probs)
