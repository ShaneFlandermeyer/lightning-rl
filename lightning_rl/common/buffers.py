...
class ICMRolloutExperience(NamedTuple):
  """
  A single policy gradient rollout experience (potentially from multiple environments) for an ICM model. Unlike the vanilla rollout experience class, this class includes the next observation.
  """
  observation: torch.Tensor
  action: torch.Tensor
...
class ICMRolloutBatch(NamedTuple):
  """
  A batch of complete rollout experiences generated by the ICM model. Unlike the vanilla rollout batch class, this class includes the predicted action (for the inverse model) and the MSE loss (for the forward model)
  """
  observations: torch.Tensor
  actions: torch.Tensor
  values: torch.Tensor
...
class ReplayBuffer():
  ...
  def append(self,
    ...
    """
    self.buffer.append(OffPolicyExperience(
...
class ICMRolloutBuffer():
  ...
  def add(
    ...
    log_prob: (torch.Tensor)
      log probability of the action following the current policy.
    """
    experience = ICMRolloutExperience(
        observation=obs,
        action=action,
  ...
  def finalize(self,
               last_values: torch.Tensor,
               last_dones: torch.Tensor) -> ICMRolloutBatch:
    """
    Finalize and compute the returns (sum of discounted rewards) and GAE advantage.
    Adapted from Stable-Baselines PPO2.
    ...
    return ICMRolloutBatch(
        ...
        advantages=advantages,
        returns=returns,
        # These get added in the collect_rollouts() function
